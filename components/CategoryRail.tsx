  ...
  return (
    <section className="mb-2">
       <div className="flex items-center px-5 mb-4">
        <h3 className="text-xl font-sans font-bold text-white tracking-wide">Categories</h3>
      </div>
            ...
            return (
                ...
                >
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 border-2 ${
                        isActive 
                        ? 'bg-[#034435] border-amber-500 text-amber-400 shadow-lg shadow-amber-900/20' 
                        : 'bg-[#022c22] border-green-800/60 text-green-400/70 hover:border-green-600'
                    }`}>
                        {getIcon(cat.id)}
                    </div>
                    <span className={`text-xs font-medium text-center truncate w-20 ${isActive ? 'text-amber-400' : 'text-green-300/60'}`}>
                        {cat.label.split(' ')[0]}
                    </span>
                </button>
            )
